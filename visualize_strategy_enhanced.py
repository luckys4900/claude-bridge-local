...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .flow-arrow {{
    text-align: center; color: #8b949e; font-size: 16px; margin: 4px 0;
  }}
  .flow-start {{
    background: #0f4c75; border-color: #58a6ff;
    color: #58a6ff; font-weight: bold;
  }}
  .flow-decision {{
    background: #d29922; border-color: #f0883e;
    color: #000; font-weight: bold;
  }}
  .flow-end {{
    background: #1c0a0a; border-color: #f85149;
  ...
  .rrr-bar {{
    height: 20px; border-radius: 3px; margin: 8px 0;
    position: relative;
  }}
  .rrr-s1 {{
  ...
  .risk-row {{
    display: flex; margin: 8px 0; align-items: center;
  }}
  .risk-label {{
...
<body>
  ...
  <!-- 新規：戦略フローチャート -->
  <h3 style="color:#8b949e;margin:16px 0 8px">📋 トレードフロー（どのように決断するか）</h3>
  <div class="flowchart">
    <div class="flow-step flow-start">開始：寄り付き前チェック</div>
    <div class="flow-arrow">↓</div>
    <div class="flow-step">流動性確認（RVOL≥1.5？）</div>
    <div class="flow-arrow">↓</div>
    <div class="flow-step flow-decision">時間帯判定</div>
    <div class="flow-arrow">↓</div>
    <div class="flow-step">09:15〜10:30：S1発動待機</div>
    <div class="flow-arrow">↓</div>
    <div class="flow-step">10:30〜11:30：C1待機（S2保持確認）</div>
    <div class="flow-arrow">↓</div>
    <div class="flow-step flow-success">12:30〜13:00：C1執行帯（唯一のRRR適合）</div>
    <div class="flow-arrow">↓</div>
    <div class="flow-step flow-end">14:00：最終判断・手仕舞い</div>
  </div>
  ...
  <!-- 新規：RRR視覚化 -->
  <h3 style="color:#8b949e;margin:16px 0 8px">📈 RRR視覚化（リスク対リターン）</h3>
  <div class="rrr-chart">
    <div class="rrr-baseline">
      <span class="rrr-baseline-label">基準ライン RRR=1.0</span>
    </div>
    <div class="rrr-bar rrr-s1">
      <span class="rrr-label">S1（窓埋め）</span>
      <span class="rrr-value">{s1_rrr}</span>
    </div>
    <div class="rrr-bar rrr-c1">
      <span class="rrr-label">C1（高精度）</span>
      <span class="rrr-value">{c1_rrr}</span>
    </div>
    <div style="margin-top:8px;font-size:11px;color:#8b949e">
      ※バーが長いほど良いRRR（リスク1に対するリターンが大きい）
    </div>
  </div>

  <!-- 新規：リスクマトリックス -->
  <h3 style="color:#8b949e;margin:16px 0 8px">⚠️ リスク評価マトリックス</h3>
  <div class="risk-matrix">
    <div class="risk-row">
      <span class="risk-label">流動性リスク</span>
      <div class="risk-bar risk-high" style="width:90%"></div>
      <span class="risk-value">90%</span>
    </div>
    <div class="risk-row">
      <span class="risk-label">期待値リスク</span>
      <div class="risk-bar risk-high" style="width:75%"></div>
      <span class="risk-value">75%</span>
    </div>
    <div class="risk-row">
      <span class="risk-label">RRR品質</span>
      <div class="risk-bar risk-medium" style="width:55%"></div>
      <span class="risk-value">55%</span>
    </div>
    <div class="risk-row">
      <span class="risk-label">時間帯制約</span>
      <div class="risk-bar risk-medium" style="width:60%"></div>
      <span class="risk-value">60%</span>
    </div>
    <div style="margin-top:8px;font-size:11px;color:#8b949e">
      ※バーが赤いほどリスクが高い。緑はリスクが低い。
    </div>
  </div>

  <h3 style="color:#8b949e;margin:16px 0 8px">📍 価格マップ（目安の価格帯）</h3>
  <div class="level-bar">
    {level_markers}
  </div>
  <div style="margin-top:8px;font-size:11px;color:#8b949e">
    ※赤＝危険ゾーン（SL）、黄＝注意ゾーン、緑＝エントリーゾーン
  </div>

  <h3 style="color:#8b949e;margin:16px 0 8px">⏰ タイムライン（時間ごとの行動）</h3>
  <div class="timeline">
    {timeline_items}
  </div>

  <h3 style="color:#8b949e;margin:16px 0 8px">🚦 Go/No-Go チェック（エントリー前の確認）</h3>
  <div class="checklist">
    {checklist_items}
  </div>

  <div class="risk-box">
    <strong>🛡️ リスク管理（損失を抑える方法）</strong><br>
    {risk_summary}
  </div>
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ファイルに保存
        output_file = sys.argv[2] if len(sys.argv) > 2 else "output/6871_strategy_card_enhanced.html"
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(html)
        print(f'ファイル保存成功: {output_file}')

        print('✅ HTML戦略カード（図解強化版）を生成しました')
